...
public class GetPriceRequestResponseFileUtils {

    public static String appendFileName = "E:\\20190808\\日志数据\\data\\get_price_request_responese.txt";
    ...
    public static void readFileByLines(String fileName) {
        ...
        try {
            ...
            int line = 1;
            String matchStr1 = "requestJsonStr";
            String matchStr2 = "responseJsonStr";
            // 一次读入一行，直到读入null为文件结束
            while ((tempString = reader.readLine()) != null) {
                // 显示行号

                if (tempString.contains(matchStr1)){
                    System.out.println("line " + line + ": " + tempString);
                    appendMethod(appendFileName, tempString.substring((tempString.indexOf(matchStr1) + matchStr1.length())
                            , tempString.length()) + "\r");

                    /*System.out.println(tempString.substring((tempString.indexOf(matchStr1) + matchStr1.length())
                            , tempString.length()) );*/
                }
